$(document).ready(function () {
  rowsEventActions("tbody_asistencias", function (action, rowId) {
    if (action == "modificar") {
      muestraMensaje(
        "¿Seguro?",
        "Seguro de que desea modificar la prima",
        "?",
        function (result) {
          if (result) {
      ...
      muestraMensaje(
        "¿Seguro?",
        "Esta seguro de que desea eliminar la prima seleccionada",
        "?",
        function (result) {
          if (result) {
            alert("eliminar");
          ...
          if (target.dataset.action == "eliminar") {
            ...
            enviaAjax(datos, function (respuesta, exito, fail) {
              ...
              if (lee.resultado == "get_user") {
                document.getElementById("modificar_id").value =
                  lee.mensaje.id_trabajador;
                document.getElementById("cedula_modificar").value =
                  lee.mensaje.cedula;
                document.getElementById("nombre_modificar").value =
                  lee.mensaje.nombre;
                document.getElementById("apellido_modificar").value =
                  lee.mensaje.apellido;
                document.getElementById("telefono_modificar").value =
                  lee.mensaje.telefono;
                document.getElementById("correo_modificar").value =
                  lee.mensaje.correo;
                document.getElementById("rol_modificar").value =
                  lee.mensaje.rol;
                document.getElementById("nivel_educativo_modificar").value =
                  lee.mensaje.id_prima_profesionalismo;
                document.getElementById("pass_modificar").value = "";
                document.getElementById("fecha_ingreso_modificar").value =
                  lee.mensaje.creado;
                document.getElementById("numero_cuenta_modificar").value =
                  lee.mensaje.numero_cuenta;

                console.log(
                  "lee.mensaje.discapacitado",
                  lee.mensaje.discapacitado
                );
                if (lee.mensaje.discapacitado == "1") {
                  document.getElementById(
                    "discapacidad_modificar"
                  ).checked = true;
                  document.getElementById("discapacidad_info_modificar").value =
                    lee.mensaje.discapacidad;
                } else {
                  document.getElementById(
                    "discapacidad_modificar"
                  ).checked = false;
                ...
                if (lee.mensaje.comision_servicios == "1") {
                  document.getElementById(
                    "comision_servicios_modificar"
                  ).checked = true;
                  document.getElementById(
                    "comision_servicios_no_modificar"
                  ).checked = false;
                } else {
                  document.getElementById(
                    "comision_servicios_no_modificar"
  ...
  $("#modal_modificar_usaurio").on("hidden.bs.modal", function () {
    $(
      "#f1_modificar input:not(input[type='radio']):not(input[type='checkbox']), #f1_modificar select"
    ).each((i, elem) => {
      elem.value = "";
  ...
  $("#modal_registar_hijos").on("hidden.bs.modal", function (e) {
    ...
    $(
      "#formularioAsistencia input:not(input[type='checkbox']):not(input[type='submit']), #formularioAsistencia select"
    ...
    document.querySelector(
      "#formularioAsistencia input[type='checkbox']"
    ).checked = false;
    ...
    document.querySelector("#modal_registar_hijos h5.modal-title").innerHTML =
      "Registrar Hijo";
  });
  ...
  document.getElementById("f1").onsubmit = function (e) {
    ...
    if (document.getElementById("f1").value != "") {
      if (this.sending) {this.sending = false; return false; }
      ...
      muestraMensaje("¿Seguro?", mensaje, "?", (resp) => {
        if (resp) {
          ...
          enviaAjax(
            datos,
            function (respuesta, exito, fail) {
              var lee = JSON.parse(respuesta);
              if (lee.resultado == "registrar") {
                muestraMensaje(
                  "Registro Exitoso",
                  "Trabajador en area ha sido registrado exitosamente",
                  "s"
                );
                ...
                muestraMensaje(
                  ...
                  "Trabajador en area ha sido modificado exitosamente",
                  "s"
                );
                $("#modal_registar_hijos").modal("hide");
              } else if (lee.resultado == "is-invalid") {
